...
struct lstack_head next;
        struct lstack_head orig; orig = __VERIFIER_load_Struct(head);
if (orig.node == 0)
        {
return 0;
        }
...
{
struct lstack_node *node; node = pop(&(*lstack).free);
if (node == 0)
        {
return ENOMEM;
        }
...
{
struct lstack_node *node; node = pop(&(*lstack).head);
if (node == 0)
        {
return 0;
        }
__VERIFIER_atomic_fetch_sub(&(*lstack).size, 1);
int value; value = (*node).value;
push(&(*lstack).free, node);
return value;
}
void *worker(void *arg)
{
int thread; thread = (int) arg;
...
int _agg_e1; _agg_e1 = lstack_pop(&results);
int _agg_e2; _agg_e2 = lstack_pop(&results);
int i1; i1 = 0;
int i2; i2 = 0;
